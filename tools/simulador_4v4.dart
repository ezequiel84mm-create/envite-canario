// Simulador de partidas 4v4 todo-IA para detectar anomalías de lógica.
// Uso:  cd ~/dev/envite_canario && dart run tools/simulador_4v4.dart
...
import 'package:envite_canario/features/multiplayer/domain/engine/trick_engine_4v4.dart';
import 'package:envite_canario/features/multiplayer/domain/engine/trick_engine_2v2.dart' show CartaJugada2v2;
import 'package:envite_canario/features/multiplayer/domain/ai/ai_player_4v4.dart';

// Equipos reales 4v4: A={0,3,4,7}, B={1,2,5,6}.
int equipoDe(int a) => (a == 0 || a == 3 || a == 4 || a == 7) ? 0 : 1;

const ordenCircular = [0, 1, 3, 2, 4, 5, 7, 6];
int siguiente(int asiento) {
  final i = ordenCircular.indexOf(asiento);
  return ordenCircular[(i + 1) % 8];
}
...
void main() {
  ...
  int ganaA = 0, ganaB = 0;
  final bazasPorAsiento = List<int>.filled(8, 0);
  int totalManos = 0;
  ...
  for (int partida = 0; partida < 50; partida++) {
    ...
    while (chicos0 < 4 && chicos1 < 4) {
      ...
      final reparto = DealEngine2v2.repartirPara(8);
      ...
      for (int numBaza = 0; numBaza < 3; numBaza++) {
        ...
        for (int paso = 0; paso < 8; paso++) {
          ...
          final validas = TrickEngine4v4.cartasValidas(
            mano: manos[asiento],
  ...
  print('Bazas ganadas por asiento (0..7): $bazasPorAsiento');
  final totalA = bazasPorAsiento[0] + bazasPorAsiento[3] + bazasPorAsiento[4] + bazasPorAsiento[7];
  final totalB = bazasPorAsiento[1] + bazasPorAsiento[2] + bazasPorAsiento[5] + bazasPorAsiento[6];
