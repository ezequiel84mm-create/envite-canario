...
class ConexionP2P {
  ...
  Future<String?> _obtenerIPLocal() async {
    try {
      ...
      // 1) Preferir la interfaz WiFi (en0) con IP privada, para evitar
      //    interfaces secundarias (VPN, Private Relay, hotspot) que dan un
      //    10.x al que el invitado no puede conectarse.
      ...
      // 2) Cualquier interfaz, priorizando 192.168.x sobre 10.x / 172.x.
      String? ip192;
      String? ipOtra;
      for (final interfaz in interfaces) {
        for (final addr in interfaz.addresses) {
      ...
      // 3) Ultimo recurso: la primera disponible.
